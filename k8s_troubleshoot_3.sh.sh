...
svcname=svc3
podname=pod3
cmname=nginx-index-cm3
volname=nginx-conf3
subpath=index3.html
owner=notHCL
indexfile=/data/index3.html
...
kubectl get nodes|grep -v "control-plane"|sed 1d|awk '{print $1" "$3}'|while read node role
do
kubectl label nodes $node owner=$owner --overwrite 2>/dev/null >/dev/null
done
